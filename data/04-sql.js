...
**พูด:** "ตรงนี้ N+1 ครับ ผมรวมเป็น query เดียวด้วย ANY แล้ว group เป็น map"`,
    note:`ดึงข้อมูลในลูป = N+1 query — รวมเป็น query เดียว (\`JOIN\`/\`IN\`/aggregate). แนวคิด: round-trip ไป DB แพงกว่าการประมวลผลต่อแถว → ลดจำนวนครั้งที่คุยกับ DB`},
...
**หลัก:** เห็นฟังก์ชันครอบคอลัมน์ใน WHERE (\`DATE()\`,\`LOWER()\`) → สงสัย index พังทันที`,
    note:`วัดด้วย \`EXPLAIN ANALYZE\` ก่อนเดา — มักขาด index หรือ query ไม่ sargable. แนวคิด: ปรับ performance ต้องวัด; index เป็น trade-off (อ่านเร็วขึ้น แต่เขียนช้าลง+กินที่)`},
...
**บทเรียน:** อย่าสับสน performance กับ correctness — เปลี่ยน join type = เปลี่ยนความหมาย`,
    note:`index เยอะ ≠ ดี — query หลายคอลัมน์พร้อมกันคิดถึง composite ก่อน. แนวคิด: planner เลือกแผนจาก statistics; index ทุกตัวมีต้นทุนตอน write`},
...
**หลัก:** เขียนหลายขั้นที่ต้อง "สำเร็จทั้งหมดหรือไม่เลย" → transaction เสมอ (+ ควรเช็ค bal ไม่ติดลบด้วย)`,
    note:`สองคำสั่งที่ต้องสำเร็จพร้อมกันต้องอยู่ใน transaction เดียว. แนวคิด: atomicity (A ใน ACID) — all-or-nothing เมื่อ crash กลางทาง; + กัน race ด้วย lock/isolation`},
...
**ข้อแลกเปลี่ยน:** keyset กระโดดไปหน้าที่ N ตรงๆ ไม่ได้ (ต้องเลื่อนต่อเนื่อง) เหมาะกับ infinite scroll / "โหลดเพิ่ม" มากกว่าเลขหน้า`,
    note:`\`OFFSET\` ลึกช้าเพราะต้อง scan ข้ามทุกแถวก่อนหน้า — ใช้ keyset/cursor (\`WHERE id > last\`). แนวคิด: OFFSET เป็น O(offset); seek ด้วยค่าที่ index ไว้เป็น O(log n)`},
   {type:"judge", title:"ตัดสินคำตอบ AI",
...
**บทเรียน:** index เยอะ ≠ ดี · query หลายคอลัมน์พร้อมกัน → คิดถึง composite ก่อน · index ทุกตัวมีต้นทุนตอน write`,
    note:`index แยกหลายตัวมักสู้ composite ที่ตรงงานไม่ได้สำหรับ query กรองหลายคอลัมน์. แนวคิด: เข้าใจว่า DB ใช้ index ยังไง (bitmap vs index scan) ก่อนตัดสินว่าดีพอ`},
...
**หลัก:** อย่าห่อ column ด้วย function/cast ใน WHERE → เขียนเป็นช่วง (range) แทน index ถึงติด`,
    note:`ห่อ column ด้วยฟังก์ชันทำให้ index ใช้ไม่ได้ (non-sargable) — เขียนเป็น range. แนวคิด: index เรียงตามค่าดิบของคอลัมน์; แปลงค่าก่อนเทียบ = ทำลายลำดับที่ index พึ่งพา`},
...
**หลัก:** นับ "จำนวนแถว" → \`COUNT(*)\` เสมอ · \`COUNT(col)\` ใช้เมื่อตั้งใจข้าม NULL`,
    note:`\`COUNT(col)\` ข้าม NULL, \`COUNT(*)\` นับทุกแถว. แนวคิด: NULL = "ไม่รู้ค่า" ไม่ใช่ 0; aggregate ส่วนใหญ่ (SUM/AVG/COUNT(col)) เมิน NULL — ต้องรู้ว่าตั้งใจหรือพลาด`}
  ]
}
);
