...
**หลัก:** ถ้าบรรทัดหลังต้องใช้ค่าที่ขึ้นกับ assertion → ใช้ \`require\``,
    note:`ใช้ \`require\` เมื่อบรรทัดถัดไปพึ่งค่านั้น (พังแล้วหยุด) — \`assert\` พังแล้วไปต่อ. แนวคิด: แยก "precondition ที่พังต้องหยุด" ออกจาก "ตรวจหลายอย่างรวดเดียว"`},
...
**บทเรียน:** AI สับสน "fail" กับ "หยุด" — assert=fail แต่ไปต่อ, require=fail แล้วหยุด`,
    note:`assert = fail-but-continue, require = fail-and-stop; ลำดับ arg (expected, actual) มีผลแค่ข้อความ. แนวคิด: อ่าน semantic ของ assertion lib ให้เข้าใจจริง อย่าเดา`},
...
Go 1.22+ แก้แล้ว แต่ในข้อสอบควรพูดถึง + ถามว่าใช้เวอร์ชันไหน`,
    note:`capture loop var ใน parallel subtest (Go < 1.22). แนวคิด: closure จับตัวแปร ไม่ใช่ค่า; concurrency + shared loop var = bug คลาสสิก`},
...
**กุญแจ:** depend on interface → mock ได้ · เทสต์ service=mock repo, เทสต์ repo=DB จริง/sqlmock`,
    note:`depend on interface เพื่อ inject mock — เทสต์ service แยกจาก DB. แนวคิด: dependency inversion ทำให้ unit อยู่ลำพังได้ (test boundary ชัด)`},
...
**หลัก:** เทสต์ห้ามพึ่งลำดับ/ผลของเทสต์อื่น`,
    note:`เทสต์ต้อง isolated — ไม่พึ่งลำดับ/ผลของกันและกัน. แนวคิด: เทสต์ที่ดีต้อง deterministic + independent; global state ทำลายทั้งสอง`},
...
- เทสต์หลาย case ด้วย table-driven (method/path/status/body)`,
    note:`ใช้ \`httptest\`/\`app.Test\` ทดสอบในหน่วยความจำ ไม่เปิด port จริง + mock dependency. แนวคิด: เทสต์ที่ boundary ของ handler โดยไม่แตะ I/O ภายนอก = เร็วและ isolated`},
...
**หลัก:** อย่า assert error ด้วยข้อความตรงตัว → \`ErrorIs\` sentinel`,
    note:`เทียบ error ด้วย \`ErrorIs\` (sentinel) ไม่ใช่ string. แนวคิด: assert บนพฤติกรรม/identity ไม่ใช่ representation ที่เปลี่ยนได้`},
...
**หลัก:** เทียบ float ห้าม \`Equal\` ตรงๆ → \`InDelta\` (ค่าสัมบูรณ์) หรือ \`InEpsilon\` (สัดส่วน เหมาะกับเลขใหญ่)`,
    note:`เทียบ float ด้วย \`InDelta\` ไม่ใช่ \`Equal\`. แนวคิด: float มี rounding error เสมอ → ความเท่ากันของ float = อยู่ในระยะ epsilon`},
...
**หลัก:** เทียบ collection ที่ลำดับไม่สำคัญ → \`ElementsMatch\` · ถ้าต้องสนลำดับค่อย \`Equal\` (หรือ sort ก่อนเทียบ)`,
    note:`\`ElementsMatch\` สำหรับ collection ที่ลำดับไม่สำคัญ (map iteration). แนวคิด: เทสต์ต้องไม่ผูกกับสิ่งที่ spec ไม่รับประกัน (เช่นลำดับ) ไม่งั้น flaky`},
...
**หลัก:** ฟังก์ชันที่รับ \`*testing.T\` แล้ว assert เอง → ใส่ \`t.Helper()\` เสมอ`,
    note:`ใส่ \`t.Helper()\` ในฟังก์ชัน assert ให้ failure ชี้จุดที่เรียกจริง. แนวคิด: error report ควรชี้ต้นเหตุที่ debug ได้ ไม่ใช่ในตัว util`},
...
**หลัก:** type จาก lib นอกที่เป็น struct (\`decimal\`, \`big.Int\`/\`big.Rat\`, \`time.Time\`) → เทียบด้วย method ของมันเอง (\`.Equal\`/\`.Cmp\`) อย่าใช้ \`assert.Equal\`/\`DeepEqual\` ที่เทียบ field ดิบ`,
    note:`type struct จาก lib นอกเทียบด้วย method ของมัน (\`.Equal\`/\`.Cmp\`) ไม่ใช่ \`DeepEqual\`; รู้ก่อนว่า \`decimal\` เป็น lib นอก. แนวคิด: equality เชิงค่า (semantic) ต่างจาก equality เชิง field (structural)`}
  ]
}
);
